...
namespace Helpers
{
    public static class Compression
    {
        ...
        public static void SimpleZip(string dirToZip, string zipName)
        {
            ...
            //DXP.Utils.PercentEndUndeterminedOperation();
        ...
        private static void Zip_SaveProgress(object sender, SaveProgressEventArgs e)
        {
            ...
            if (e.TotalBytesToTransfer > 0 & !initialized)
            {
                ...
                DXP.Utils.PercentInit("Copying Project", (int)e.TotalBytesToTransfer/ 32768);
                initialized = true;
                transfer = e.TotalBytesToTransfer;
            }
            if (initialized)
            {
                if (e.TotalBytesToTransfer != transfer) initialized = false;
                DXP.Utils.PercentUpdate();
                //Last = e.BytesTransferred;
            }
        }
